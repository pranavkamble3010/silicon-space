...
switch(state)
{
	...
	not_on_path = true;
	//player_direction = point_direction(x,y,obj_main_player.x,obj_main_player.y);
	player_direction = point_direction(x,y,obj_main_player.x,obj_main_player.y+80);
	image_angle = player_direction-270;
	path_end();
	//move_towards_point(obj_main_player.x+200,obj_main_player.y+200,3);
	//move_towards_point(obj_main_player.x+200,obj_main_player.y-70,3);
	if(reload==0)
	{
	...
	case enemy_state.patrol :	
		if(not_on_path)
		{
			path_start(path2,15,path_action_restart,true);
			not_on_path = false;
		}
